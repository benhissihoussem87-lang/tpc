...
//Get Adresse Client By Facture
$adressesClient=$facture->get_All_AdressesClient_ProjetsFacture($_GET['facture']);
$nbAdresse=count($adressesClient);

// Verification si la facture est forfitaire ou non 
$verifForfitaireFacture=null;$verifMultiAdresseFacture=null;
if(!empty($ProjetsFacture)){foreach($ProjetsFacture as $projet){
	if($projet['prixForfitaire']!=''){$verifForfitaireFacture='forfitaire';}
	else if($projet['prix_unit_htv']!=''){$verifForfitaireFacture='Nonforfitaire';}
	if($projet['adresseClient']!=''){$verifMultiAdresseFacture='oui';}
	else {$verifMultiAdresseFacture='non';}
}}
//echo '<h1 style="text-align:center"> Etat Facture '.$verifForfitaireFacture.'</h1>';
...
<?php 
//echo '<h1 style="padding-top:250px"> '.$verifForfitaireFacture.'<br>Multi Adresse '.$verifMultiAdresseFacture.'</h1>';
