...
<th valign=top colspan=2 style="border:solid 3px black;"><h2 style="padding-top:10px;font-size:18px">BUREAU DE CONTROLE TECHNIQUE ET EXPERTISE</h2></th>
</tr>
...
<tr><td></td><th   style="font-size:15px">M.F: 1541179 V/A/M/000  </th>
<th style="border:2px solid black;font-size:15px">M.F:  <input type="text" > </th></tr>
<tr><td></td><th   style="font-size:15px">R.I.B :STB 10020.134100229078830</th><td></td></tr>
...
if($det['qte1']!=0 or $_GET['Pourcentage']!=0){?>
<tr >
<td>1</td>
<td > <?php if($det['qte1']!=0 or $_GET['Pourcentage']!=0){ echo @$detBRD['article1'];}?></td>
<td> <?php if(@$_GET['Pourcentage']!=0){ echo $_SESSION['qte1Brd'];} else if($det['qte1']!=0){echo $det['qte1'];}?></td>
<td > <?php if($det['qte1']!=0 or @$_GET['Pourcentage']!=0){ echo @$detBRD['prix1'];}?></td>

<td> <?php if(@$_GET['Pourcentage']!=0){ echo $_SESSION['qte1Brd']*@$detBRD['prix1'];} else if($det['qte1']!=0)
{echo @$det['qte1']*@$detBRD['prix1'];}?></td>

</tr>
<?php @$tot=$tot+($det['qte1']*$detBRD['prix1']); }} 
if($detBRD['prix2']!=0 and $detBRD['prix2']!= ""){
if($det['qte2']!=0 or @$_GET['Pourcentage']!=0){

$tot=$tot+($det['qte2']*$detBRD['prix2']);
?>
<tr>
<td>2</td>
<td><?php if($det['qte2']!=0 or @$_GET['Pourcentage']!=0){ echo $detBRD['article2'];}?></td>
<td> <?php if(@$_GET['Pourcentage']!=0){ echo $_SESSION['qte2Brd'];} else if($det['qte2']!=0){echo $det['qte2'];}?></td>
<td> <?php if($det['qte2']!=0 or @$_GET['Pourcentage']!=0){ echo $detBRD['prix2'];}?></td>

<td> <?php if(@$_GET['Pourcentage']!=0){ echo $_SESSION['qte2Brd']*@$detBRD['prix2'];} else if($det['qte2']!=0)
{echo @$det['qte2']*@$detBRD['prix2'];}?></td>
...
<tr>
<td>3</td>
<td><?php if($det['qte3']!=0 or @$_GET['Pourcentage']!=0){ echo $detBRD['article3'];}?></td>
<td> <?php if(@$_GET['Pourcentage']!=0){ echo $_SESSION['qte3Brd'];} else if($det['qte3']!=0){echo $det['qte3'];}?></td>
<td> <?php if($det['qte3']!=0 or @$_GET['Pourcentage']!=0){ echo $detBRD['prix3'];}?></td>
<td> <?php if(@$_GET['Pourcentage']!=0){ echo $_SESSION['qte3Brd']*@$detBRD['prix3'];} else if($det['qte3']!=0)
{echo @$det['qte3']*@$detBRD['prix3'];}?></td>
...
<tr height=30 style="position:relative;top:0px"><td colspan=3>Arrêter la présente facture à la somme de : <b style="text-transform:uppercase"><?php echo asLetters($totale).' '?> <input type="text" style="width:30px;border:none;font-weight:bold;font-size:14px;text-align:center" >  <?php echo 'millimes';?></b></td></tr>
<tr height=30><td colspan=3>Demeurant à votre  entière disposition pour tous  renseignements complémentaires,  nous vous prions de croire, Messieurs, l’expression de notre parfaite considération </td></tr>
<tr height=30><td></td><td></td><th align=center  style="position:relative;top:0px;"> SIGNATURE ET CACHET<br><span style="width:20px;margin:auto">"SIGMA CONTROLE"</span></th></tr>


</table>
<table  width=80% align="center" height=20 style="border:solid 3px black;position:relative;top:100px;">
