...
{ 
		...
		$offres=$AO->AfficherOffreMDO($_POST['rechercheMDO']);
		}
		
		$_SESSION['mdoOffre']=$_POST['rechercheMDO'];
		
}//Fin isset($_POST['btnrechercheMDO']
else if(isset($_SESSION['mdoOffre']))
			{
					  if($_SESSION['mdoOffre']=='All')
					  {
					  $offres=$AO->getAppelOffre();
					  }
					  else{
					$offres=$AO->AfficherOffreMDO($_SESSION['mdoOffre']);
					}
			}
...
if($_GET['AO_etat']==2)
{
 ...
 {
	 $AO->deleteBorderaux($_GET['codeAo']);
  ...
  <!--Fin Modal statiqtique-->
          ...
          <div class="card mb-3">
            <div class="card-header">
              ...
              <b style="font-size:20px;position:relative;top:30px;left:-20px">OFFRE DE PRIX</b></td>
			  <td>
			  <table style="position:relative;left:160px; top:-30px"  width=90% >
			  ...
			  <form method="post" style="width:220px;">
				 ...
				 <?php if(isset($_SESSION['mdoOffre'])){?>
				      <option selected><?php echo $_SESSION['mdoOffre']?></option>
				 <?php } ?>
				 
				  <option>All</option>
				  <?php if(count($mdoOffre)>0){


                     foreach($mdoOffre as $md){
				  ?>
			  ...
			  <td><a href="?AO_Ajout" style="width:150px; ; margin:auto;position:relative; top:0px;"   class="btn btn-primary btn-block" >Ajout</a></td>
			  <?php if($_SESSION['user']=='Admin'){?>
			  <td>
			   <a href=""  style="width:150px; ; margin:auto;position:relative; top:0px;"  name="ok" class="btn btn-primary btn-block" data-toggle="modal" data-target="#exampleModal" data-whatever="@mdo">Statistique</a>
			   </td>
			   <!--  Export Excel -->
			  <td><a href="Core/Export.php?tableName=offres" style="width:150px; ; margin:auto;position:relative; top:0px;"   class="btn btn-primary btn-block" >Export Excel</a></td>
			 <?php } ?>
			  <td>
			  <form method="post">
			  <table>
			  ...
			  if(!isset($_GET['codeAO']))
                 {
						...
						/* else if(!isset($_POST['btn_etat'])){
						 @$offres=$AO->getAppelOffre();
						 }*/
				 
				 ?>
                <table class="table table-bordered" id="dataTable" width="100%" height=200>
                  <thead >
                    <tr>
                      <th >Code OP</th>
					   <th>Code P</th>
				  ...
				  {
					 $typeBorderaux=$AO->getTypeBorderaux($cle['code']);
					 @$nbBrds=count($typeBorderaux);
					 if($nbBrds>0)
					 {
					  foreach($typeBorderaux as $typeBRD);
					  }
					  ?>
                    <tr <?php  if($cle['Typeuser']=='utilisateur'){?> bgcolor="yellow"<?php } ?>>
					  ...
					  <?php echo $cle['code']?>
					  <?php } else if($_SESSION['user']=='Admin' or $_SESSION['user']=='Admin2' ) {?>
					  <a href="?AO_Affiche&codeAO=<?php echo $cle['code']?>&typeAO=<?php echo $cle['type']?>" class="btn btn-outline-info" style="padding:2px 8px;font-size:12px">
					  <b>OP</b><?php echo $cle['code']?></a>
					  <br>
					  <?php if($cle['bordero']!=null){?>
					  <a href="Appel_offres/bordero/<?php echo $cle['bordero']?>" target="_blank">
					  Bordereau</a>
					  <?php }?>
					  <?php }?>
					   ...
					   <td>
					    <?php
						if($cle['type']!='ponctuel' and $cle['type']!='PUBLIC PONCTUEL' )
						{
						 ...
						 {
						    $ponctuelsOP=$ponct->getPonctuelOP($cle['code']);
							@  $nb_poncts=count($ponctuelsOP);
							 // echo '<h2>'.$nb_poncts.'<h2>';
							  if($nb_poncts>0)
							  {
							  foreach($ponctuelsOP as $pnct);
							  
							  
							  echo '<b>PP</b>'.$pnct['code'].'<br>';
                      ...
                      <td>
					  <?php echo $cle['type']?><br>
					  <?php 
					  @$nbTypes=count($typeBorderaux);
					   if($nbTypes>0)
					 {
					  if($typeBRD['type']=='expertise'){
					  echo '<b>'. $typeBRD['type'].'</b>';
					  ...
					  ?></td>
					   <td><?php
                    if($cle['type']=='prive' or $cle['type']=='ponctuel' )
					  {echo $cle['date_decharge'];}
					else
					{
					   echo $cle['date_limite'];
					   }
					   ?>
					  ...
					  <td align="left"><a href="?AO_Affiche&codeAO=<?php echo $cle['code']?>&typeAO=<?php echo $cle['type']?>&AO_modifier=<?php echo $cle['id']?>&etatAO=<?php echo $cle['etat']?>">
					  <img src="images/update.jpg" width="25" height="25"></a>
					  </td>
					  <td align="left">
					<?php if($cle['code']!='00000'){?>
					<a href="?AO_Affiche&codeAo=<?php echo $cle['code']?>&AO_delete=<?php echo $cle['id']?>&AO_etat=<?php echo $cle['etat']?>">
					<img src="images/delete.jpg" width="25" height="25"></a>
					<?php } ?>
					</td>
                     <?php }else {?>
					...
					<td align="left">
					<a href="?AO_Affiche&codeAO=<?php echo $cle['code']?>&typeAO=<?php echo $cle['type']?>&AO_modifier=<?php echo $cle['id']?>&etatAO=<?php echo $cle['etat']?>"><img src="images/update.jpg" width="25" height="25"></a>
					 </td>
					  <td align="left">
					  <?php if($cle['code']!='00000'){?>
					  <a href="?AO_Affiche&codeAo=<?php echo $cle['code']?>&AO_delete=<?php echo $cle['id']?>&AO_etat=<?php echo $cle['etat']?>">
		  ...
		  <th width=30% >Code Offre de Prix</th><td><b>OP</b><?php echo $cdAO['code']?></td>
		  ...
		  <th>Pourcentage</td><td><?php 
					  ...
					  ?></td>
					  <th>Fichier de Bordereau</th><td><a href="Appel_offres/bordero/<?php echo $cdAO['bordero']?>" target="_blank">Bordero<?php echo $_GET['codeAO']?></a></td>
		  </tr>
		     <tr><th>Vis à Vis</td><td colspan=3><?php echo $cdAO['vis_a_vis']?></td></tr>
			...
			$brds=$borderos->getborderoOP($_GET['codeAO']);
				...
				<tr><td></td><td></td><td>Qte</td><td>Prix</td></tr>
			<tr>
			
			
			<tr>
			
			<td width=10%>Art.1</td><td><?php echo $cle['article1']?></td>
			<td><?php echo $cle['qte1']?></td><td><?php echo $cle['prix1']?></td>
			</tr> 
			...
			<td><?php echo $cle['qte2']?></td><td><?php echo $cle['prix2']?></td>
			</tr> 
			...
			<td><?php echo $cle['qte3']?></td><td><?php echo $cle['prix3']?></td>
			</tr> 
			...
			<td><?php echo $cle['qte4']?></td><td><?php echo $cle['prix4']?></td>
			</tr> 
			...
			<td><?php echo $cle['qte5']?></td><td><?php echo $cle['prix5']?></td>
			</tr> 
